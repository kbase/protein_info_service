...
# Authentication: none
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function fids_to_operons_local (received $n, expecting 1)");
    }
    {
	my($fids) = @args;
    ...
    my $result = $self->{client}->call($self->{url}, {
	method => "ProteinInfo.fids_to_operons_local",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method fids_to_operons_local",
					    status_line => $self->{client}->status_line,
					    method_name => 'fids_to_operons_local',
				       );
    }
...
# Authentication: none
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function fids_to_orthologs_local (received $n, expecting 1)");
    }
    {
	my($fids) = @args;
    ...
    my $result = $self->{client}->call($self->{url}, {
	method => "ProteinInfo.fids_to_orthologs_local",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method fids_to_orthologs_local",
					    status_line => $self->{client}->status_line,
					    method_name => 'fids_to_orthologs_local',
				       );
    }
